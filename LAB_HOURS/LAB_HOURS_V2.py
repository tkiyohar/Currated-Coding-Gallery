...
credentials = ServiceAccountCredentials.from_json_keyfile_name(
    "humans-lab-hours-00b010fa4155.json", scope
)
...
def help():
    ...
    print(
        '"/endday": Ends the current day and discounts the hours of anyone who is still signed in (make sure everyone who is here has signed out before ending the day).'
    )
...
def DayZDonE():
    ...
    print(
        "Day concluded. The following people forgot to sign out and have thus forfeited their hours:"
    )
...
while i == 0:
    ...
    while nameinputsection == True:
        ...
        if name in Name_Row:
            ...
            print(
                "Hello ",
                name,
                ', if this is your first time signing in enter "y" or enter "n" to re-enter your name as it may be incorrect, (please note: consistent capitalization is important).',
            )
            firstsignin = input()
            connectionCheck()
            if firstsignin == "y":
    ...
    if Signintime:
        ...
        SignInTimeInSeconds = int(
            3600 * (SignInHours) + 60 * (SignInMinutes) + (SignInSeconds)
        )
        SignOutTimeInSeconds = int(
            3600 * (SignOutHours) + 60 * (SignOutMinutes) + (SignOutSeconds)
        )
        if 18000 in range(SignInTimeInSeconds, SignOutTimeInSeconds):
            ...
            print(
                "Sorry but it appears your forgot to sign out yesterday and thus your lab hours haven't been recorded. If you believe this to be an error please inform either Oscar or Trent otherwise please re-enter your name to sign in."
            )
            ...
            TotalSignInTime = (
                SignOutTimeInSeconds - SignInTimeInSeconds + EarlierSignInTime
            )
